...
export const InstructorInterface = () => {
  ...
  if (!currentUser || currentUser.role !== 'instructor' && !isAdmin) {
    return <div className="min-h-screen bg-gradient-to-br from-purple-600 via-indigo-500 to-blue-500 p-6 flex items-center justify-center">
  ...
  return <div className="min-h-screen bg-gradient-to-br from-purple-600 via-indigo-500 to-blue-500 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        {!isAdmin && !currentUser?.attractionId && <Card className="mb-6 bg-yellow-50 border-yellow-200">
            <CardContent className="p-6 text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {isAdmin && <Card className="bg-white/95 backdrop-blur-sm">
              <CardHeader>
              ...
              <CardContent>
                <Select value={selectedAttraction} onValueChange={setSelectedAttraction}>
                  ...
                  <SelectContent>
                    {attractions.filter(a => a.isActive).map(attraction => <SelectItem key={attraction.id} value={attraction.id}>
                ...
                {attraction && <div className="mt-4 p-3 bg-gray-50 rounded-lg">
        ...
        {attraction && <Card className="mt-6 bg-white/95 backdrop-blur-sm">
            <CardHeader>
            ...
            <CardContent>
              {queue.length === 0 ? <div className="text-center py-8 text-gray-500">
                  ...
                  {queue.map((entry, index) => <div key={entry.id} className={`flex items-center justify-between p-4 rounded-lg border-2 ${index === 0 ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
                      <div className="flex items-center">
                        <div className={`rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold mr-4 ${index === 0 ? 'bg-green-600 text-white' : 'bg-blue-600 text-white'}`}>
                          {index + 1}
                        </div>
                        <div>
                      ...
                      <Button onClick={() => handleCompleteRide(entry.braceletCode, entry.customerName)} className="bg-green-600 hover:bg-green-700 text-white" size="lg">
